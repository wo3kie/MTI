...
const Value* Program::execute() {
    ...
    runTimeData.activationRecord=
        &_tablicaFunkcji.value( std::make_pair( Scope::globalScope, funkcjaGlownaPozycja))->activationRecord();
    ...
    for( int parameter= 0; parameter< runTimeData.activationRecord->valueList.size(); ++parameter){
        if( runTimeData.activationRecord->valueList[ parameter]->type()!=
        parameters.argv[ parameter]->type()){
            throw IException(std::string("Cannot call function: ")+ _funkcjaGlowna);
        }
    }
